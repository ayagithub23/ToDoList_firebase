...
function render(todoData) {
    ...
    for (var i = 0; i < todoData.length; i++) {
       ...
       ${todoData[i].content}
       <span class="date-right">${moment(new Date()).format("MM/DD HH:mm")}</span>
        </li>`
...
database.ref("todoData").once("value").then(function (snapshot) {
    var $ul = $("ul");
    var html = "";
    snapshot.forEach(function (childsnapshot) {
        ...
        var data = `<li id="${value.id}"><button class="delete">刪除</button>
        ${value.content}<span class="date-right">${moment(value.createdAt).format("MM/DD HH:mm")}</span></li>`;
        // html = html + data;
        html += data;
    });
    $ul.append(html);
...
// database.ref("address").set({country:"tw",city:"taipei"}).then(function(snapshot){
//     console.log("OK");
// })
